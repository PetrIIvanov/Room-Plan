...
def draw_2d_plan(ax):
    ...
    # Дверь ОТКРЫТА (от X=-8 до X=67, центрирована на Y=34)
...
def draw_3d_view(ax, elev, azim, title):
    ...
    # Дверь ЗАКРЫТА (в плоскости X=-8, от Y=34 до Y=109, Z=0 до Z=222)
    ...
    # Дверь ОТКРЫТА (от X=-8 до X=67, центрирована на Y=34, толщина 4)
    ax.add_collection3d(Poly3DCollection(
...
# ============ ГЕНЕРАЦИЯ HTML С ПРАВИЛЬНОЙ ДВЕРЬЮ ============
...
<head>
    ...
    <style>
        ...
        .images { display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 20px; margin: 20px 0; }
        .image-box { text-align: center; padding: 10px; background: #fafafa; border-radius: 4px; }
        .image-box img { max-width: 100%; height: auto; border: 1px solid #ddd; border-radius: 4px; cursor: pointer; transition: transform 0.2s; }
        ...
        .lightbox-content { margin: auto; display: block; max-width: 95%; max-height: 95%; object-fit: contain; }
        .lightbox-close { position: absolute; top: 20px; right: 40px; color: #fff; font-size: 40px; font-weight: bold; cursor: pointer; z-index: 10000; }
...
<body>
    ...
    <script>
        ...
        // ЛЕВАЯ СТЕНА С ПРОЁМОМ (2 части)
        // Нижняя часть: от Z=0 до Z=34
        scene.add(createBox(0, 0, 0, """ + str(wall_thickness) + """, """ + str(ceiling_height) + """, """ + str(door_gap_bottom) + """, materials.leftWall));

        // Верхняя часть: от Z=109 до Z=153
        scene.add(createBox(0, 0, """ + str(door_gap_bottom + door_width) + """, """ + str(wall_thickness) + """, """ + str(ceiling_height) + """, """ + str(room_height - door_gap_bottom - door_width) + """, materials.leftWall));

        // ДВЕРЬ ОТКРЫТА (от X=-8 до X=67, центрирована на Y=34, толщина 4)
        scene.add(createBox(""" + str(hinge_x) + """, 0, """ + str(hinge_y - door_thickness/2) + """, 
                           """ + str(door_leaf) + """, """ + str(ceiling_height) + """, """ + str(door_thickness) + """, materials.door));
...
print("   - index.html (интерактивная 3D модель)")
print("\n🔧 Исправления:")
print("   ✅ Проём в левой стене (между Z=34 и Z=109)")
print("   ✅ Дверь открыта, начинается от петли (-8) и идёт до X=67")
print("   ✅ Комната не отзеркалена")
